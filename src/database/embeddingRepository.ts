...
import { float32ArrayToBase64, base64ToFloat32Array, base64ToHex, hexToBase64 } from '../utils/serialization';
...
export async function insertEmbedding(userId: number, embedding: Float32Array, version: string): Promise<void> {
  ...
  const hex = base64ToHex(encrypted);
...
export async function getEmbedding(id: number): Promise<EmbeddingRow | null> {
  ...
  if (result && result.length > 0 && result[0].rows.length > 0) {
    ...
    const encryptedBase64 = hexToBase64(row.embedding_hex);
    const decryptedBase64 = await decryptData(encryptedBase64);
