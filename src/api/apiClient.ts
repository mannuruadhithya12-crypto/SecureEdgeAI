...
const BACKEND_URL = Platform.OS === 'android' ? 'http://192.168.1.4:5000' : 'http://localhost:5000';
...
function validateUrlSecurity(url: string): void {
  const isDevHost = url.includes('localhost') || url.includes('10.0.2.2') || url.includes('127.0.0.1') || url.includes('192.168.');
